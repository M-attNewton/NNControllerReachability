...
include("NNPsatzChorFuncsPoly.jl")

# Read NN from file
global net = matopen("PPPendulumWeights.mat")
...
#        end
        if abs(Y_max_curr_layer[k]) > abs(Y_min_curr_layer[k])
             ...
             global ineq_cons = vcat(ineq_cons, (x_curr_layer[k] - (X_max_curr_layer[k]/Y_max_curr_layer[k])*v[k])*(v[k] - x_curr_layer[k]))

        elseif abs(Y_max_curr_layer[k]) < abs(Y_min_curr_layer[k])

             global ineq_cons = vcat(ineq_cons, (x_curr_layer[k] - (X_min_curr_layer[k]/Y_min_curr_layer[k])*v[k])*(v[k] - x_curr_layer[k]))
...
elseif AF == "poly1"
    for j in 1:size(dim_hidden,2)
       ...
       for k in 1:dim_hidden[j]
        ...
        #      global ineq_cons = vcat(ineq_cons, (x_curr_layer[k] - (X_max_curr_layer[k]/Y_max_curr_layer[k])*v[k])*(v[k] - x_curr_layer[k]))
        # end
            global eq_cons = vcat(eq_cons, x_curr_layer[k]*((v[k])^2 + 4) - 4*v[k])
...
global x_tplus12 = u[2] + delta_t*(-mass*grav*leng*q + mass*grav*leng*u[1] - mu*u[2] + v_out)/(mass*(leng)^2)
global x_tplus1 = vcat(x_tplus1, x_tplus12)

if abs(u_max[1]) > abs(u_min[1])
    global grad_z3 = (u_max[1] - sin(u_max[1]))/abs(u_max[1])
elseif abs(u_max[1]) < abs(u_min[1])
    global grad_z3 = (u_min[1] - sin(u_min[1]))/(u_min[1])
else
    global grad_z3 = (u_max[1] - sin(u_max[1]))/abs(u_max[1])
end
#global ineq_sys = q*(grad_z3*u[1] - q

#global ineq_cons = vcat(ineq_cons, (v_out - sat_max)*(sat_max - v_out))
global ineq_cons = vcat(ineq_cons, (v_out[1] + sat_max))
global ineq_cons = vcat(ineq_cons, (sat_max - v_out[1]))
global ineq_cons = vcat(ineq_cons, q*(grad_z3*u[1] - q))
#global ineq_cons = vcat(ineq_cons, 0.063 - q)
#global ineq_cons = vcat(ineq_cons, 0.063 + q)
#global ineq_cons = vcat(ineq_cons, )

...
function intBoundProp(u_min,u_max,dims,dim_hidden,weights,biases,AF)
    ...
    for k in 2:(size(dim_hidden,2)+1)
        ...
        elseif AF == "poly1"
            for j in 1:size(z_mintemp,1)
                z_mintemp[j] = (4*z_mintemp[j])/((z_mintemp[j])^2 + 4)
                z_maxtemp[j] = (4*z_maxtemp[j])/((z_maxtemp[j])^2 + 4)
            end
        end
...
function intBoundProp2(u_min,u_max,dims,dim_hidden,weights,biases,AF)
    z_min = u_min
    z_max = u_max
    y_min = z_min
    y_max = z_max
